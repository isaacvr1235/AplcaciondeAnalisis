...
class DataLoader:
    ...
    def cargar_excel(self, ruta, nombre=None, hoja=None, **kwargs):
        """Carga un archivo .xlsx / .xls y devuelve un Dataset."""
        if not os.path.exists(ruta):
            raise FileNotFoundError(f"Archivo no encontrado: {ruta}")

        # Si no se especifica hoja, listar las disponibles
        xls = pd.ExcelFile(ruta)
        hojas = xls.sheet_names

        if hoja is None and len(hojas) > 1:
            print(f"  Hojas disponibles: {hojas}")
            print(f"  Cargando la primera hoja: '{hojas[0]}'")
            hoja = hojas[0]
        elif hoja is None:
            hoja = hojas[0]

        df = pd.read_excel(ruta, sheet_name=hoja, **kwargs)
        nombre = nombre or f"{os.path.basename(ruta)}[{hoja}]"
        ds = Dataset(
            nombre=nombre,
            datos=df,
            origen=ruta,
        ...
        print(f"  ✔ Excel cargado: '{nombre}'  [{df.shape[0]} filas x {df.shape[1]} columnas]")
        return ds

    # ── JSON ──────────────────────────────────

    def cargar_json(self, ruta, nombre=None, **kwargs):
        """Carga un archivo .json y devuelve un Dataset."""
        if not os.path.exists(ruta):
            raise FileNotFoundError(f"Archivo no encontrado: {ruta}")
        ...
        ds = Dataset(
            ...
            datos=df,
            origen=ruta,
            metadata={"formato": "JSON", "ruta_absoluta": os.path.abspath(ruta)}
        )
        print(f"  ✔ JSON cargado: '{nombre}'  [{df.shape[0]} filas x {df.shape[1]} columnas]")
        return ds
    ...
    def cargar(self, ruta, **kwargs):
        ...
        if ext == ".csv":
            ...
            return self.cargar_excel(ruta, **kwargs)
        elif ext == ".json":
            return self.cargar_json(ruta, **kwargs)
        else:
            raise ValueError(
                f"Formato no soportado: '{ext}'. "
                f"Formatos válidos: .csv, .tsv, .txt, .xlsx, .xls, .json"
            )
